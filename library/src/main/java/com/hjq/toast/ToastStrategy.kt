 ...
 */
class ToastStrategy @JvmOverloads constructor(
    /** 吐司显示策略  */
    ...
    override fun showToast(params: ToastParams) {
        when (mShowStrategyType) {
            SHOW_STRATEGY_TYPE_IMMEDIATELY -> {
                ...
                val uptimeMillis =
                    SystemClock.uptimeMillis() + params.delayMillis + (if (params.crossPageShow) 0 else DEFAULT_DELAY_TIMEOUT)
    ...
    protected fun isSupportToastStyle(toast: IToast?): Boolean {
        ...
        return toast is CustomToast || VERSION.SDK_INT < VERSION_CODES.R || mApplication!!.applicationInfo.targetSdkVersion < VERSION_CODES.R
    }
    ...
    protected fun diyToastStyle(toast: IToast, style: IToastStyle<*>) {
        ...
        toast.setMargin(style.horizontalMargin, style.verticalMargin)
    }
